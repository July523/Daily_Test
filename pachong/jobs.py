...
def get_json(kind, page=1, ):
    ...
    # 设置代理
    proxies = {'http': '140.143.96.216:80'}


    # 请求的url
    url = 'https://www.lagou.com/jobs/positionAjax.json?' \
          'px=default&city=%E5%8C%97%E4%BA%AC&needAddtionalResult=false'
    # 使用代理访问
...
# 将总数据转化为data frame再输出
    df = pd.DataFrame(data=search_job_result,
                      columns=['公司全名', '公司简称', '公司规模',
                               '融资阶段', '区域', '职位名称', '工作经验', '学历要求', '工资', '职位福利'])
    df.to_csv('lagou2.csv', index=False, encoding='utf-8_sig')
