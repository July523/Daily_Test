# coding=utf-8
"""
    GAN网络用于MNIST数据集的简单测试，具体代码来源于：
https://blog.csdn.net/jiongnima/article/details/80033169?utm_source=blogxgwz2
"""
...
def save(savers, sess, logdir, step):  # 保存模型的save函数
    model_name = 'model'  # 模型名前缀
    checkpoint_path = os.path.join(logdir, model_name)  # 保存路径
    savers.save(sess, checkpoint_path, gloal_step=step)  # 保存模型
    print('The checkpoint has been created.')


def xavier_init(size):  # 初始化参数时使用的xavier_init函数
    in_dim = size[0]
    xavier_stddev = 1. / tf.sqrt(in_dim / 2.)  # 初始化标注差
...
def generator(z):  # 生成器，z的维度为[N, 100]
    ...
    G_prob = tf.nn.sigmoid(G_log_prob)  # G_log_prob经过一个sigmoid函数
...
Z_dim = 100  # 生成器输入的随即噪声的列的维度

mnist = input_data.read_data_sets('C:/Users/Administrator/AppData/Local'
                                  '/Programs/Python/Python36/Lib/site-packages'
                                  '/tensorflow/MNIST_data/', one_hot=True)


sess = tf.Session()  # 会话层
sess.run(tf.global_variables_initializer())  # 初始化所有可训练的参数
...
for it in range(1000000):  # 训练100万次
    if it % 1000 == 0:  # 每训练1000次就保存一下结果
        ...
        fig = plt.plot(samples)  # 通过plot函数生成可视化结果
